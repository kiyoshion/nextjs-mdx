---
title: 'Laravel + React + InertiaではじめるSPA開発 〜VPSにSSRでデプロイしてみる〜'
desc: 'これはLaravel + React + InertiaではじめるSPA開発のチュートリアルです。'
intro: 'これはLaravel + React + InertiaでSPAを開発するチュートリアルです。Laravelのインストール、React、Inertiaの導入、VPSにSSRでデプロイするまでの手順を網羅しています。話せば長くなるので、事前に要件の確認をおすすめします。'
...
## ゴール
LaravelとReactでSPAを開発し、VPS（LEMP）にデプロイしてSSR（OGP対応）させる。

## Laravelのインストール
Laravelをインストールします。すでにLaravelの初期設定が完了し、http://localhost:8000にアクセスできる状態ならスキップしてください。
...
### MySQLの作成
データベースを作成します。すでにLaravelとデータベースが接続済み（Laravel Sail、PHP MyAdmin、XAMPP、MAMMPなど）ならスキップしてください。
...
試しにユーザーを登録してみましょう。ユーザー登録が成功し、dashboardページに遷移したら成功です。


ユーザー登録が成功するとdashboradページに遷移
後の工程で画像アップロード機能を実装します。ストレージのシンボリックリンクを作成しておきましょう。
...
APP_KEY=
APP_DEBUG=false
APP_URL=https://laravel-react-inertia.chichack.com
...
```nginx
npm install -g pm2
pm2 start bootstrap/js/ssr.jsx --name laravel_inertia -i 1
pm2 list
```
